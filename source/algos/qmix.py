...
class QMix:
	def __init__(self, args):
		...
		elif args.alg == 'qmix+commnet' and not args.arcomm: # for commnet
		...
		if self.args.cuda:
			...
			self.eval_qmix_net.cuda(device=self.args.cuda_device)
			self.target_qmix_net.cuda(device=self.args.cuda_device)
			if args.arcomm:
		...
		if args.arcomm:
			...
			self.eval_parameters = list(self.eval_qmix_net.parameters()) + list(self.eval_rnn.parameters()) + list(self.arcomm.parameters()) + list(self.decoder_reg.parameters())
		else:
	...
	def learn(self, batch, max_episode_len, train_step, epsilon=None):
		...
		obs, state, state_next, actions, reward, avail_actions, avail_actions_next, terminated = batch['obs'], batch['state'], batch['state_next'], \
		...
		if self.args.cuda:
			...
			state = state.cuda(device=self.args.cuda_device)
			actions = actions.cuda(device=self.args.cuda_device)
			reward = reward.cuda(device=self.args.cuda_device)
			state_next = state_next.cuda(device=self.args.cuda_device)
