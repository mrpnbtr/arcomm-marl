...
# commnet smac
class CommNetRolloutWorker_SMAC:
	def __init__(self, env, agents, args):
		self.env = env
		self.agents = agents
		self.episode_limit = args.episode_limit
		...
		print("CommNetRolloutWorker_SMAC initialized")
	...
	def generate_episode(self, episode_num=None, evaluate=False, epoch_num=None, eval_epoch=None): 
		...
		while not terminated and step < self.episode_limit: 
			...
			action = self.agents.choose_action_vdn_qmix(obs, last_action, avail_action, epsilon, evaluate)

			action_onehot = np.zeros((self.args.n_agents, self.args.n_actions))
			...
			# need to reshape the list of actions into a vector with shape (n_agents, 1) to store in the buffer
			actions_ep.append(np.reshape(actions.numpy(), [self.n_agents, 1]))
			actions_onehot_ep.append(actions_onehot)
			avail_actions_ep.append(avail_actions)
			reward_ep.append([reward])  
			terminate.append([terminated]) 
...
class CommRolloutWorker:
	...
	def generate_episode(self, episode_num=None, evaluate=False, epoch_num=None, eval_epoch=None):
		...
		while not all(terminated): 
			...
			avail_action = np.array([[1] * self.n_actions for _ in range(self.n_agents)])
...
class CommRolloutWorker2:
	def __init__(self, env, agents, args):
		...
		print("CommRolloutWorker-TEST initialized")
...
class CommRolloutWorker2_SMAC:
	def __init__(self, env, agents, args):
		...
		print("CommRolloutWorker-TEST-SMAC initialized")
